    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            with open(filename,'w') as file:
                file.write(f"{timestamp} - {len(processes)} processes\n")
                file.write("="*25)
                ...
                for proc in processes:
                    ...
                    file.write(f"{pid}\t{name}\t{username}\t{cpu}\t{mem}\t{phys_mem}\t{exe}\t{cmdline}")
            return processes
        return wrapper
    return decorator
...
def filter_by_current_user(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        processes = func(*args, **kwargs)
        filtered_processes = []
        print(f"[Filtering] getting only procceses for user: {current_user}")
        for proc in processes:
        ...
        print(f"[Filtering] Filtered from {original_count} - {filtered_count} processes")
